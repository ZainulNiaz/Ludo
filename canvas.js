...
var bigRect = new Rectangle(0.3 * (window.innerWidth), 0.1 * (window.innerHeight), 0.8 * (window.innerHeight), 0.8 * (window.innerHeight), '#0000FF00');
bigRect.draw();
...
for (var i = 0; i < 4; i++) {
  // c.beginPath();
  // c.rect(x, y, 0.4 * (lengthofboard), 0.4 * (lengthofboard));
...
for (var i = 1; i <= 48; i++) {
  if (i % 12 == 2) {
    ...
    rectArray.push(new Rectangle(boxdrawx, boxdrawy, smallsquaresize, smallsquaresize, '#0000FF00'));
  }
...
for(var i=0; i<5;i++){
  ...
  leaderboardtext.push(new text(textx+0.15*canvas.height, texty, '30px serif', 'dummytext', 'black'));
...
for(var i=0;i<16;i++){
  ...
  if(i<4){noofchipscompleted.push(0);}
}
...
function movechips() {
  ...
  this.move = function() {
    ...
    for (var i = 0; i < chipsArray.length; i++) {
      ...
      k+=Math.floor(sx / 64);
      k+=1;
      if (Math.abs(chipsArray[i].x - mouse.x) < chipsArray[i].radius && Math.abs(chipsArray[i].y - mouse.y) < chipsArray[i].radius && j * 4 < i + 1 && j * 4 > i - 4 && !moveafterclick) {
        moveafterclick= true;
        if (chipsArrayposition[i] == 0 && Math.floor(sx/64) == 5) {
          ...
          noofmovesofchip[i]++;
        } else if (chipsArrayposition[i] == 0) {} else if (noofmovesofchip[i] + Math.floor(sx/64) > 57) {}
        else if (noofmovesofchip[i] + Math.floor(sx/64) == 57) {noofchipscompleted[Math.floor(i/4)]++;}
...
turnrect.draw();
var turntext = "green";
if(noofdiceclick%4==0){turntext="green";}
if(noofdiceclick%4==1){turntext="yellow";}
if(noofdiceclick%4==2){turntext="blue";}
if(noofdiceclick%4==3){turntext="red";}
...
function animate() {
  ...
  if(noofdiceclick%4==0){turnrecttext.text="green"; sy=64*4;}
  if(noofdiceclick%4==1){turnrecttext.text="yellow"; sy= 64*3;}
  if(noofdiceclick%4==2){turnrecttext.text="blue"; sy = 64*5;}
  if(noofdiceclick%4==3){turnrecttext.text="red"; sy = 64*1;}
